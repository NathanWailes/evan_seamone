...
def extract_the_orders_into_new_files_in_a_new_directory(base_path, every_nth_case, debug=False):
    path_to_input_cases = os.path.join(base_path,
                                       '03 case files separated based on whether they have an ORDER section',
                                       'has an ORDER section')
    base_path_for_output = os.path.join(base_path, '04 isolated orders sections')
    ...
    for index, filename in enumerate(os.listdir(path_to_input_cases)):
        ...
        with open(path_to_file, 'r', encoding='utf-8') as infile:
            ...
            if len(regex_matches) >= 1:
                output_orders_section_to_a_new_file(base_path_for_output, filename, regex_matches)
    ...
    if debug:
        print_results(cases_we_didnt_find_the_order_section_for, matches_against_the_regex,
                      cases_without_the_string_ORDER_in_it, cases_with_multiple_matches, number_of_cases_considered)


def get_order_section(case_text):
    regex_to_find_order_section = r"\n\s*ORDERS?\s*\n\s*(.+?)\s*\n\s*(?:(?:(?:[^\n]+\n)?[^\n]+\n[^\n]+Board\sof\sVeterans['’]\sAppeals)|(?:______)|(?:[A-Z\ \.\:]{3,}?\s*\n))"
    regex_matches = re.findall(regex_to_find_order_section, case_text, re.DOTALL)
    return regex_matches


def output_orders_section_to_a_new_file(base_path_for_output, filename, regex_matches):
...
def output_the_list_of_cases_that_we_didnt_find_an_order_section_for(base_path, cases_we_didnt_find_the_order_section_for):
    path_to_output_file = os.path.join(base_path, '04 cases we didnt find the order section for.txt')
    with open(path_to_output_file, 'w') as outfile:
        for filename in cases_we_didnt_find_the_order_section_for:
            outfile.write(filename + '\n')


def print_results(cases_we_didnt_find_the_order_section_for, matches_against_the_regex,
                  cases_without_the_string_ORDER_in_it, cases_with_multiple_matches, number_of_cases_considered):
    print("\n\n***\n")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Extract the orders sections from cases into new files in a '
                                                 'separate directory.')
    ...
    parser.add_argument('--debug', type=int,
                        help='This will print information about what was done / found.  1 for True, default '
                             'is False')
    ...
    extract_the_orders_into_new_files_in_a_new_directory(base_path, every_nth_case, debug)
